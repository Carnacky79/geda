...
function calcolaContatore() {
     ...
     if(tipologia==1) {
          ...
          if(anno!=undefined) {
               ...
               var ajax_obj = {pag:'contatori', action:'getRecord', redirect:false, wait:false, obj:{ nome:toJSON('ddt_'+tipologia), anno:toJSON(anno) }, esegui:function(data) {
                    $('#codice').val(data);
               }};
               ajax_call(ajax_obj);
...
function salva_valori() {
     ...
     if(errori_righe===false && !$.isEmptyObject(obj.obj_righe)) {
          ...
          var ajax_obj = {pag:'ddt', action:'saveRecord', redirect:true, wait:true, obj:{ valori:valori, obj:toJSON(obj) }, esegui:function(data) {
               location.href = '?pagina=ddt_edit&id_ddt='+data+'&type='+tipologia;
          }};
          ajax_call(ajax_obj);
     } else {
