...
$(document).on('change', ' #id_fornitore, #id_cliente ', function(){
     ...
     var righe =  $('#tab_ddt_righe tbody tr');
     ...
     righe.each(function() {
          ...
          var table = $('#tab_ddt_righe').DataTable();
          table.row( $('#tab_ddt_righe tbody tr[id_riga="'+id_riga+'"]') ).remove().draw();
     });
...
$(document).on('change', ' [name="id_oggetto"] ', function(){
     ...
     if(id_oggetto.split('_').length==2) {
          ...
          var ajax_obj = {pag:'ddt', action:'getUltimo_prezzo_and_iva', redirect:false, wait:false, obj:{ id_ddt:toJSON(id_ddt), id_categoria_tipologia:toJSON(id_oggetto.split('_')[0]), id_prodotto_servizio:toJSON(id_oggetto.split('_')[1]), id_fornitore:toJSON($('#id_fornitore').val()), id_cliente:toJSON($('#id_cliente').val()) }, esegui:function(data) {
...
function carica() {
     ...
     function aggiungi() {
          ...
          $('#tab_ddt_righe tbody tr').each(function(){
     ...
     function calcola() {
          ...
          $('#tab_ddt_righe tbody tr').each(function() {
               ...
               $('#tab_ddt_info tbody').removeClass('hidden');
          ...
          $.each(obj_iva, function(key,val){
               $('.info_ddt').append("IVA "+record_iva[key].nome+": Imponibile € "+parseFloat(val.imp).toFixed(3)+" - IVA € "+parseFloat(val.iva).toFixed(3)+"<br/>");
          });
     }
