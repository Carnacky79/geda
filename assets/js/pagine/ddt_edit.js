...
function salva_valori() {
     ...
     if(errori_righe===false && !$.isEmptyObject(obj.obj_righe)) {
          ...
          var ajax_obj = {pag:'ddt', action:'updateRecord', redirect:true, wait:true, obj:{ valori:valori, obj:toJSON(obj) }, esegui:function(data) {
               location.href = '?pagina=ddt_list';
          }};
          ajax_call(ajax_obj);
     } else {
...
$(document).on('click', '.form_header .delete_this', function(){
     ...
     if(confirm('Confermare la cancellazione del DDT corrente?')) {

          var valori = raccogli_valori();
          valori = toJSON(valori);


          var ajax_obj = {pag:'ddt', action:'deleteRecord', redirect:true, wait:true, obj:{ valori:valori }, esegui:function(data) {
               location.href = '?pagina=ddt_list';
          }};
          ajax_call(ajax_obj);
     }
