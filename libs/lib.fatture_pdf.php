...
class fatture_pdf extends baseclass {
	...
	public static function getFatturePdfInstance() {
	...
	public function get($id_fattura) {
		...
		$fattura = $this->fatture->getRecord($id_fattura);
		$righe = $this->fatture->getRighe($id_fattura);


		if ($fattura->id_fornitore>0) {
			$fornitore = $this->fornitori->getRecord($fattura->id_fornitore);
		} else {
		...
		if ($fattura->id_cliente>0) {
			$cliente = $this->clienti->getRecord($fattura->id_cliente);
		} else {
		...
		$tabella_fattura = $this->crea_tabella_fattura($fattura);
		$tabella_totali = $this->crea_tabella_totali($fattura);
		$tabella_dettaglio_iva = $this->crea_tabella_dettaglio_iva($calcoli->dettaglio_iva);
		$tabella_note = $this->crea_tabella_note($fattura);
		...
		$pdf->writeHTMLCell(50, 50, 10, 50, $tabella_cliente); $pdf->writeHTMLCell(50, 50, 150, 50, $tabella_fattura);
		...
		$file_name = "Fattura ".$fattura->codice." del ".$fattura->data;
	...
	public function crea_tabella_titolo() {
		...
		<tr>
		<td style=\" $this->no_border font-weight:bold; font-size:45px; color:orange; \" >FATTURA</td>
		</tr>
		<tr>
	...
	public function crea_tabella_fattura($fattura) {

		$table = "<table style=\" $this->no_border \" cellpadding=\"0\" >
		<tbody>

		<tr nobr=\"true\" >
		<td style=\" $this->bold \" >Fattura n.</td>
		<td style=\" $this->right \" >$fattura->codice</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold \" >Data</td>
		<td style=\" $this->right \" >".utility::toUIDate($fattura->data)."</td>
		</tr>
	...
	public function crea_tabella_note($fattura) {

		return nl2br($fattura->descrizione);
	}
