...
class ddt_pdf extends baseclass {
	...
	public static function getddtPdfInstance() {
	...
	public function get($id_ddt) {
		...
		$ddt = $this->ddt->getRecord($id_ddt);
		$righe = $this->ddt->getRighe($id_ddt);


		if ($ddt->id_fornitore>0) {
			$fornitore = $this->fornitori->getRecord($ddt->id_fornitore);
		} else {
		...
		if ($ddt->id_cliente>0) {
			$cliente = $this->clienti->getRecord($ddt->id_cliente);
		} else {
		...
		$tabella_ddt = $this->crea_tabella_ddt($ddt);
		$tabella_totali = $this->crea_tabella_totali($ddt);
		$tabella_dettaglio_iva = $this->crea_tabella_dettaglio_iva($calcoli->dettaglio_iva);
		$tabella_note = $this->crea_tabella_note($ddt);
		...
		$pdf->writeHTMLCell(50, 50, 10, 50, $tabella_cliente); $pdf->writeHTMLCell(50, 50, 150, 50, $tabella_ddt);
		...
		$pdf->writeHTMLCell(115, 50, 10, $y, $tabella_dettaglio_iva); $pdf->writeHTMLCell(70, 50, 130, $y, $tabella_totali);
		...
		$file_name = "DDT ".$ddt->codice." del ".$ddt->data;
	...
	public function crea_tabella_titolo() {
		...
		<tr>
		<td style=\" $this->no_border font-weight:bold; font-size:45px; color:orange; \" >DDT</td>
		</tr>
		<tr>
	...
	public function crea_tabella_ddt($ddt) {

		$table = "<table style=\" $this->no_border \" cellpadding=\"0\" >
		<tbody>

		<tr nobr=\"true\" >
		<td style=\" $this->bold \" >DDT n.</td>
		<td style=\" $this->right \" >$ddt->codice</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold \" >Data</td>
		<td style=\" $this->right \" >".utility::toUIDate($ddt->data)."</td>
		</tr>
	...
	public function crea_tabella_totali($totali) {
		...
		<td style=\" $this->right $this->size_2 \" >€ ".number_format($totali->imponibile,3,",",".")."</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold $this->size_1 \" >IVA</td>
		<td style=\" $this->right $this->size_2 \" >€ ".number_format($totali->iva,3,",",".")."</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold $this->size_1 \" >Esenti</td>
		<td style=\" $this->right $this->size_2 \" >€ ".number_format($totali->esente,3,",",".")."</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold $this->size_1 \" >Spese accessorie</td>
		<td style=\" $this->right $this->size_2 \" >€ ".number_format($totali->spese_acc,3,",",".")."</td>
		</tr>

		<tr nobr=\"true\" >
		<td style=\" $this->bold $this->size_1 \" >TOTALE</td>
		<td style=\" $this->right $this->size_2 \" >€ ".number_format($totali->totale,3,",",".")."</td>
		</tr>
	...
	public function crea_tabella_dettaglio_iva($dettaglio_iva) {
		...
		foreach ($dettaglio_iva as $id_iva => $valore) {
			...
			$html_dettaglio_iva .=" IVA ".$this->record_iva->{$id_iva}->nome.": Imponibile € ".number_format($valore->imp,3,",",".")." - IVA € ".number_format($valore->iva,3,",",".")."<br/>";
	...
	public function crea_tabella_note($ddt) {

		return nl2br($ddt->descrizione);
	}
